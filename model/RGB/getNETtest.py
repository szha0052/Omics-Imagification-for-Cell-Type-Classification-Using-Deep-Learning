import torch
import numpy as np
from conv_pixel import conv_pixel
...
class NetTester:
    ...
    def __init__(self, dset, Out, device='cpu'):
        ...
        self.Ytest = np.array(dset['test_labels'])          # (n_samples,)
    ...
    def run_test(self):
        ...
        n_samples = self.Ytest.shape[0]
        XTest_np = np.zeros((n_samples, 1, self.xp, self.yp), dtype=np.float32)
        for j in range(n_samples):
            # MATLAB: ConvPixel(dset.Xtest(:, j), ...)
            image_2d = self.conv_pixel(self.Xtest[:, j])
