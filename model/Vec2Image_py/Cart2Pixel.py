...
def Cart2Pixel(Q, A=None, B=None):
    ...
    if Q['Method'].lower() == 'kpca':
        ...
        if data.shape[0] < 5000:
            ...
            perplexity = Q.get('perplexity', min(30, data.shape[0] - 1))
            Y = TSNE(n_components=2, perplexity=perplexity, metric=Q['Dist']).fit_transform(data)
           
        else:
            print('tSNE with barnes-hut algorithm is used')
            perplexity = Q.get('perplexity', min(30, data.shape[0] - 1))
            Y = TSNE(n_components=2, algorithm='barnes_hut', perplexity=perplexity, metric=Q['Dist']).fit_transform(data)
    ...
    for p1 in range(n):
        for p2 in range(p1 + 1, n):
            if p1 >= z.shape[1] or p2 >= z.shape[1]:
                continue
